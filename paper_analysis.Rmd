...
plot_metrics_excerno <- function (tbl, metrics, threshold) {
  ...
  ggplot(new_tbl, aes(x = ffpe_cos, y = mean))+
    ...
    geom_point(data = fixed_below, color = "red")+
    geom_point(data = fixed_above, color = "deepskyblue")+
...
Let's combine both datasets in a single graph, coloring by signature
...
rep_ffpe_cos <- rep(ffpe_cosmic_cos_sims, each=length(pct_ffpe)*3)
...
We are interested in plotting the performance characteristics across multiple dillution levels so we define a function to do that:
...
Here is the information for each of the performance characteristics:
...
## Another way to look at it (Omit)

We know that

* Sensitivity is a decreasing function of percent FFPE.
* Specificity is an increasing function of percent FFPE.

We would like to summarize this characteristics more succintly and establish the relationship with the similarity against the FFPE signature. To do that we will we fix a threshold value in 80% and:

* We will record the maximum ffpe where sensitivity drops below this threshold. We will call this **maximum permissible sensitivy FFPE* 
* We will record the first percent ffpe where specificity rises above this threshold **minimum permissible specificity FFPE* 

```{r}
threshold <- 0.8

sens_thres_tbl <- sim_dillution_tbl %>%
  filter (metric == "Sensitivity") %>%
  filter(mean>threshold) %>%
  group_by (sbs) %>%
  slice (which.max(pct_ffpe)) %>%
  ungroup() %>%
  mutate(cos = ffpe_cosmic_cos_sims)

spec_thres_tbl <- sim_dillution_tbl %>%
  filter (metric == "Specificity" ) %>%
  filter (mean > threshold) %>%
  group_by (sbs) %>%
  slice (which.min(pct_ffpe)) %>%
  ungroup() %>%
  mutate(cos = ffpe_cosmic_cos_sims)
```

Let's look at the distribution of those thresholds
...
ggplot(sens_thres_tbl, aes(x=pct_ffpe))+
  geom_histogram(binwidth=5, color="black", fill="lightblue")+
  labs(x= "Percent of FFPE signature",
       y="Number")
    

ggplot(spec_thres_tbl, aes(x=pct_ffpe))+
  geom_histogram(binwidth=5, color="black", fill="lightblue")+
  labs(x= "Percent of FFPE signature",
       y="Number")
...
And let's plot the relationship with similarity against the FFPE signature:

```{r}
sens_thres_tbl %>%
  ggplot (aes(cos, pct_ffpe)) +
  geom_point() + 
  geom_smooth(method="lm", formula = "y~x")+
  scale_x_continuous(breaks = scale_y, labels = scale_y)+
  scale_y_continuous(breaks = scale_x, labels = scale_x)+
  labs(x= "Cosine similarity against FFPE",
         y="Extreme percent FFPE")
...
And the linear model metrics for each case:

```{r}
sens_thres_lm <- lm (pct_ffpe ~ cos, sens_thres_tbl)

spec_thres_lm <- lm (pct_ffpe ~ cos, spec_thres_tbl)

summary(sens_thres_lm)
summary(spec_thres_lm)
```
