...
for (x in 1:length(signature.names)){
  ...
  for (y in 1:10){
    ...
    classification.df.1 <- 
      get_classification_df(list(sample.vec, ffpe.sample.vec), 
                            c(signature.names[x],"FFPE"), 
                            list(signature.matrix.list[[x]], ffpe.signature))
...
ggplot(cos.sim.auc.data, aes(x=average_auc)) +
  geom_histogram(color="black", fill="lightblue")+
  labs( x = "Average AUC", y = "Count")
#ggsave("supp.fig.5.png", dpi = 300, width = 175, height = 108, units = "mm")

median(cos.sim.auc.data$average_auc)
...
cos.sim.auc.outliers <- cos.sim.auc.data %>%
  filter(average_auc<0.8) 

cos.sim.auc.outliers

cos.sim.auc.outliers2 <- cos.sim.auc.data %>%
  filter(average_auc>=0.8 & average_auc<0.9)

cos.sim.sbs4 <- cos.sim.auc.data %>%
  filter(signature=="SBS4")
...
ggplot(cos.sim.auc.data, aes(x = cosine_similarity, y = average_auc))+
  geom_point()+
  geom_smooth(method = "lm", se=FALSE)+
  labs( x = "Cosine similarity between baseline COSMIC and FFPE signatures", y = "Average AUC")+
  geom_point(data = cos.sim.auc.outliers, color = "red")+
  geom_point(data = cos.sim.auc.outliers2, color = "purple")+
  ...
  geom_label_repel(data = cos.sim.auc.outliers2, aes(label = signature), max.overlaps = 10, label.size =   0.1)+
  geom_label_repel(data = cos.sim.sbs4, aes(label = signature), max.overlaps = 10, label.size =   0.1)
  

#ggsave("Fig2b.png", dpi = 300, width = 175, height = 108, units = "mm")
...
ggplot(sens.spec.data, aes(x=sensitivity)) +
  geom_histogram(color="black", fill="lightblue")+
  labs( x = "Average sensitivity", y = "Count")
#ggsave("supp.fig.3.png", dpi = 300, width = 175, height = 108, units = "mm")

median(sens.spec.data$sensitivity)
IQR(sens.spec.data$sensitivity)


outliers <- sens.spec.data %>%
  filter(sensitivity<0.8 | signature=="SBS4" | signature=="SBS6")

ref.signatures <- sens.spec.data %>%
  filter(signature=="SBS4" )


ggplot(sens.spec.data, aes(x = cosine_similarity, y = sensitivity))+
  geom_point()+
  geom_smooth(method = "lm", se=FALSE)+
  labs( x = "Cosine similarity between baseline COSMIC and FFPE signatures", y = "Average sensitivity")+
  geom_point(data = outliers, color = "red")+
  geom_hline(yintercept=0.8, linetype="dashed", color = "red")+
  geom_label_repel(data = outliers, aes(label = signature), max.overlaps = 10, label.size =   0.1)


#ggsave("supp.fig6.png", dpi = 300, width = 175, height = 108, units = "mm")


model.sens <- lm(sensitivity ~ cosine_similarity, data = sens.spec.data)

summary(model.sens)


ggplot(sens.spec.data, aes(x=specificity)) +
  geom_histogram(color="black", fill="lightblue")+
  labs( x = "Average specificity", y = "Count")
#ggsave("supp.fig.4.png", dpi = 300, width = 175, height = 108, units = "mm")

median(sens.spec.data$specificity)
IQR(sens.spec.data$specificity)

outliers <- sens.spec.data %>%
  filter(specificity<0.8 | signature=="SBS4" | signature=="SBS6")


ggplot(sens.spec.data, aes(x = cosine_similarity, y = specificity))+
  geom_point()+
  geom_smooth(method = "lm", se=FALSE)+
  labs( x = "Cosine similarity between baseline COSMIC and FFPE signatures", y = "Average specificity")+
  geom_point(data = outliers, color = "red")+
  geom_hline(yintercept=0.8, linetype="dashed", color = "red")+
  geom_label_repel(data = outliers, aes(label = signature), max.overlaps = 10, label.size =   0.1)


#ggsave("supp.fig7.png", dpi = 300, width = 175, height = 108, units = "mm")


model.spec <- lm(specificity ~ cosine_similarity, data = sens.spec.data)
