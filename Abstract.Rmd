...
MMR.perf <- performance(test.pred, measure = "tpr", x.measure = "fpr")
tob.pred <- prediction(tob.ROC$Tobacco, tob.ROC$truthTob)
tob.perf <- performance(tob.pred, measure = "tpr", x.measure = "fpr")
plot(MMR.perf, colorize=TRUE, print.cutoffs.at=0.5, text.adj=c(-0.2,1.7),
     main = "ROC curve of classifier", lty = 2) #ROC
plot(tob.perf, colorize=TRUE, print.cutoffs.at=0.5, text.adj=c(-0.2,1.7), 
     add = TRUE, lty = 3)
abline(a=0, b= 1)
legend("bottom", c("MMR with FFPE noise", "Tobacco with FFPE noise"),
       lty = 2:3)
```
...
```{r}
num.mut=1000
probabilities.MMR <- vector(mode = "list", length = 99)
truth.MMR <- vector(mode = "list", length = 99)

for (x in seq(10, num.mut-1, by = 10)){
  sample.ffpe <- create_signature_sample_vector(ffpe.sig, x)
  ffpe.truth <- rep("FFPE", x)
  sample.sig6 <- create_signature_sample_vector(cosmic.sig6, num.mut-x)
  sig6.truth <- rep("MMR", num.mut-x)
  ...
  probabilities.MMR[[x/10]] <- classification$MMR
  truth.MMR[[x/10]] <- classification$trueBinary
}
```

```{r}
num.mut=1000
probabilities.tob <- vector(mode = "list", length = 99)
truth.tob <- vector(mode = "list", length = 99)

for (x in seq(10, num.mut-1, by = 10)){
  sample.ffpe <- create_signature_sample_vector(ffpe.sig, x)
  ffpe.truth <- rep("FFPE", x)
  sample.sig4 <- create_signature_sample_vector(cosmic.sig4, num.mut-x)
  sig4.truth <- rep("Tobacco", num.mut-x)
  ...
  probabilities.tob[[x/10]] <- classification$Tobacco
  truth.tob[[x/10]] <- classification$trueBinary
}
```
...
```{r}
AVG.sens.MMR <- vector(mode = "list", length = 99)

for (iteration in 1:50){
  sens.MMR <- performance(test.MMR, measure = "sens")
  
  for(ind in 1:length(sens.MMR@x.values)){
    thresh = as.vector(slot(sens.MMR, "x.values")[[ind]])
    sens = as.vector(slot(sens.MMR, "y.values")[[ind]])
    if (length(thresh) > 2){
      if (identical(which(thresh<=0.5), integer(0))){
        AVG.sens.MMR[[ind]][iteration] = NA
      }
      else{
        ...
        AVG.sens.MMR[[ind]][iteration] = slope*(0.5 - thresh[low.ind]) + sens[low.ind]
      }
    }
    else{
      AVG.sens.MMR[[ind]][iteration] = NA
...
sens.MMR.vec = c()
for (avg in 1:length(AVG.sens.MMR)){
  sens.MMR.vec[avg] = mean(AVG.sens.MMR[[avg]], na.rm = TRUE)
}
```
...
```{r}
AVG.sens.tob <- vector(mode = "list", length = 99)

for(iteration in 1:50){
  sens.tob <- performance(test.tob, measure = "sens")
  
  for(ind in 1:length(sens.tob@x.values)){
    thresh = as.vector(slot(sens.tob, "x.values")[[ind]])
    sens = as.vector(slot(sens.tob, "y.values")[[ind]])
    if (length(thresh) > 2){
      if (identical(which(thresh<=0.5), integer(0))){
        AVG.sens.tob[[ind]][iteration] = NA
      }
      else{
        low.ind = max(which(thresh >=0.5))
        high.ind = min(which(thresh <=0.5))
        #point slop formula of a line
        slope = (sens[high.ind] - sens[low.ind]) / (thresh[high.ind] - thresh[low.ind])
        AVG.sens.tob[[ind]][iteration] = slope*(0.5 - thresh[low.ind]) + sens[low.ind]
      }
    }
    else{
      AVG.sens.tob[[ind]][iteration] = NA
...
sens.tob.vec = c()
for (avg in 1:length(AVG.sens.tob)){
  sens.tob.vec[avg] = mean(AVG.sens.tob[[avg]], na.rm = TRUE)
}
```

### Sensitivity across signatures

```{r}
sensitivity = append(sens.MMR.vec, sens.tob.vec)
signature.sens = append(rep("MMR", length(sens.MMR.vec)),
                        rep("Tobacco", length(sens.tob.vec)))
dilution = append(1:99, 1:99)
...
```{r}
AVG.spec.MMR <- vector(mode = "list", length = 99)

for(iteration in 1:20){
  spec.MMR <- performance(test.MMR, measure = "spec")
  
  for(ind in 1:length(spec.MMR@x.values)){
    thresh = as.vector(slot(spec.MMR, "x.values")[[ind]])
    spec = as.vector(slot(spec.MMR, "y.values")[[ind]])
    if (length(thresh) > 2){
      if (identical(which(thresh<=0.5), integer(0))){
        AVG.spec.MMR[[ind]][iteration] = NA
      }
      else{
        low.ind = max(which(thresh >=0.5))
        high.ind = min(which(thresh <=0.5))
        #point slop formula of a line
        slope = (spec[high.ind] - spec[low.ind]) / (thresh[high.ind] - thresh[low.ind])
        AVG.spec.MMR[[ind]][iteration] = slope*(0.5 - thresh[low.ind]) + spec[low.ind]
      }
    }
    else{
      AVG.spec.MMR[[ind]][iteration] = NA
...
spec.MMR.vec = c()
for (avg in 1:length(AVG.spec.MMR)){
  spec.MMR.vec[avg] = mean(AVG.spec.MMR[[avg]], na.rm = TRUE)
}
```
...
```{r}
AVG.spec.tob <- vector(mode = "list", length = 99)

for (iteration in 1:20){
  spec.tob <- performance(test.tob, measure = "spec")
  
  for(ind in 1:length(spec.tob@x.values)){
    thresh = as.vector(slot(spec.tob, "x.values")[[ind]])
    spec = as.vector(slot(spec.tob, "y.values")[[ind]])
    if (length(thresh) > 2){
      if (identical(which(thresh<=0.5), integer(0))){
        AVG.spec.tob[[ind]][iteration] = NA
      }
      else{
        low.ind = max(which(thresh >=0.5))
        high.ind = min(which(thresh <=0.5))
        #point slop formula of a line
        slope = (spec[high.ind] - spec[low.ind]) / (thresh[high.ind] - thresh[low.ind])
        AVG.spec.tob[[ind]][iteration] = slope*(0.5 - thresh[low.ind]) + spec[low.ind]
      }
    }
    else{
      AVG.spec.tob[[ind]][iteration] = NA
...
spec.tob.vec = c()
for (avg in 1:length(AVG.spec.tob)){
  spec.tob.vec[avg] = mean(AVG.spec.tob[[avg]], na.rm = TRUE)
}
```

### Specificity across all signatures

```{r}
specificity = append(spec.MMR.vec, spec.tob.vec)
signature.spec = append(rep("MMR", length(sens.MMR.vec)),
                        rep("Tobacco", length(spec.tob.vec)))
dilution = append(1:99, 1:99)
...
```{r}
AVG.sens.MMR <- vector(mode = "list", length = length(step))

for (iteration in 1:100){
  sens.MMR <- performance(load.MMR, measure = "sens")
...
load.sens.MMR.vec = c()
for (avg in 1:length(AVG.sens.MMR)){
  load.sens.MMR.vec[avg] = mean(AVG.sens.MMR[[avg]], na.rm = TRUE)
}
```

```{r}
AVG.sens.tob <- vector(mode = "list", length = length(step))

for (iteration in 1:100){
  sens.tob <- performance(load.tob, measure = "sens")
...
load.sens.tob.vec = c()
for (avg in 1:length(AVG.sens.tob)){
  load.sens.tob.vec[avg] = mean(AVG.sens.tob[[avg]], na.rm = TRUE)
}
```
...
```{r}
AVG.spec.MMR <- vector(mode = "list", length = length(step))

for (iteration in 1:100){
  spec.MMR <- performance(load.MMR, measure = "spec")
...
load.spec.MMR.vec = c()
for (avg in 1:length(AVG.spec.MMR)){
  load.spec.MMR.vec[avg] = mean(AVG.spec.MMR[[avg]], na.rm = TRUE)
}
```

```{r}
AVG.spec.tob <- vector(mode = "list", length = length(step))

for (iteration in 1:100){
  spec.tob <- performance(load.tob, measure = "spec")
...
load.spec.tob.vec = c()
for (avg in 1:length(AVG.spec.tob)){
  load.spec.tob.vec[avg] = mean(AVG.spec.tob[[avg]], na.rm = TRUE)
}
```
