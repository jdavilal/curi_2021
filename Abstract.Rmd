...
for (x in seq(10, num.mut-1, by = 10)){
  sample.sig6 <- create_signature_sample_vector(cosmic.sig6, x)
  sig6.truth <- rep("MMR", x)
  sample.ffpe <- create_signature_sample_vector(ffpe.sig, num.mut-x)
  ffpe.truth <- rep("FFPE", num.mut-x)
...
for (x in seq(10, num.mut-1, by = 10)){
  sample.sig4 <- create_signature_sample_vector(cosmic.sig4, x)
  sig4.truth <- rep("Tobacco", x)
  sample.ffpe <- create_signature_sample_vector(ffpe.sig, num.mut-x)
  ffpe.truth <- rep("FFPE", num.mut-x)
...
```{r}
plot(1:99, sens.MMR.vec, ylim = c(0,1), xlab = "Percent of each signature", 
     ylab = "Sensitivity value", type = "l", main = "Comparing performance characteristics")
lines(1:99, sens.tob.vec, lty = 2)
abline(h = 0.85, col = "red")
legend("bottomright", c("MMR sensitivity", "Tobacco sensitivity", 
...
```{r}
plot(1:99, spec.MMR.vec, ylim = c(0,1), xlab = "Percent of each signature", 
     ylab = "Specificity value", type = "l", main = "Comparing performance characteristics")
lines(1:99, spec.tob.vec, lty = 2)
abline(h = 0.85, col = "red")
legend("bottomleft", c("MMR specificity", "Tobacco specificity", 
